...
export function SiteFooter() {
  return <footer className="footer"><div className="footer-brand"><Image src="/brand/vivlox-wordmark.png" alt="Vivlox — Define Your Essence" width={215} height={72} /><p>PRE-MADE. CREATIVE.<br />READY TO WEAR.</p></div>{[["SHOP","New Drops","Collections","Denim","All Products"],["HELP","FAQ","Shipping","Returns","Track Order"],["VIVLOX","About","Community","Drop Alerts","Instagram"],["LEGAL","Privacy","Terms","Accessibility"]].map(([title,...links])=><div key={title}><h3>{title}</h3>{links.map((label)=><a href={label === "Privacy" ? "/privacy" : label === "Terms" ? "/terms" : "/#shop"} key={label}>{label}</a>)}</div>)}<div className="footer-bottom">© 2026 VIVLOX. ALL RIGHTS RESERVED. <span>BUILT WITH THREAD. WORN WITH ATTITUDE.</span></div></footer>;
}
